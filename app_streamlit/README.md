...
현재 문서는 **프론트엔드 v1.2** 구현을 기준으로 작성되었습니다.
...
- 신청 가이드에서 검색된 정책을 우선 선택
- FastAPI 챗봇 API 연동
- 현재 앱 버전 `v1.2` 표시

독립된 `상세 분석` 페이지는 v1.2에서 삭제되었습니다. 상세 정보는 추천 결과
...
```text
4th_project/
├─ KakaoTalk_20260612_161223810.png  # 로고 및 브라우저 아이콘
...
기본 프로필은 내부 상태 초기화에 사용합니다. 검색하지 않은 상태의 추천 결과
필터에는 나이와 지역이 선택되지 않은 상태로 표시됩니다.
...
- 추천 결과에서 검색된 정책을 선택 목록에 우선 표시
- 팝업에서 이동한 경우 해당 정책을 기본 선택
- 신청 기간과 현재 상태 표시
- 신청 절차 5단계 표시
- 공식 사이트 링크 제공

검색 결과가 없으면 전체 정책이 선택 목록에 표시됩니다.

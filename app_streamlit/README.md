...
현재 문서는 **프론트엔드 v1.5** 구현을 기준으로 작성되었습니다.
...
- 첫 진입 시 정책 데이터 로드 중 `assets/policy_loading.png` 기반 로딩 화면 표시
...
데이터는 `st.cache_data`로 캐시됩니다. 정책 목록이 필요 없는 페이지 전환에서는
대용량 정책 JSON 로드를 건너뛰고, 원본에 없는 정보는 임의로 만들지 않고
...
- 첫 진입 시 정책 데이터 로드 중 동일한 로딩 화면을 먼저 표시
...
- 앱 표시 버전 `v1.5`로 변경
- 챗봇 화면 제목을 `정책 챗봇`에서 `안쉼 챗봇`으로 변경
- 첫 진입 시 정책 데이터 로드 중 백구 로딩 화면을 표시하고 Streamlit 기본 툴바 숨김 처리 보강
